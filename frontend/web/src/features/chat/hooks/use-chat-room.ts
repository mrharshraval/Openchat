...
export function useChatRoom(sessionId: string, session: Session | null) {
  ...
  const [isStrangerDisconnected, setIsStrangerDisconnected] = React.useState(false)
  ...
  const messages = useChatStore((state) => state.messagesByChatId[sessionId]) ?? []

  const setMessages = React.useCallback((updater: Message[] | ((prev: Message[]) => Message[])) => {
    if (typeof updater === "function") {
      useChatStore.getState().setMessages(sessionId, updater(useChatStore.getState().messagesByChatId[sessionId] || []))
    } else {
      useChatStore.getState().setMessages(sessionId, updater)
    }
  }, [sessionId])
  ...
  const handleSend = React.useCallback((textareaRef: React.RefObject<HTMLTextAreaElement | null>) => {
    ...
    if (editingMsg) {
      ...
      useChatStore.getState().updateMessage(sessionId, editingMsg.id || editingMsg.clientMessageId || "", {
        content: text,
      })
      setEditingMsg(null)
    } else {
      ...
      useChatStore.getState().appendMessage(sessionId, optimisticMsg)
      ...
      useChatStore.getState().updateConversation(sessionId, {
  ...
  React.useEffect(() => {
    ...
    const handleChatHistory = (payload: any) => {
      ...
      useChatStore.getState().setMessages(sessionId, history)
      if (payload.partnerNickname) setPeerNickname(payload.partnerNickname)
      if (payload.partnerUsername) setPeerUsername(payload.partnerUsername)
      if (payload.selfId) {
    ...
    const handleMessage = (payload: any) => {
      ...
      useChatStore.getState().appendMessage(sessionId, newMsg)
    ...
    const handleMessagePersisted = (payload: any) => {
      useChatStore.getState().updateMessage(sessionId, payload.clientMessageId, { id: payload.id, status: "PERSISTED" })
    }

    const handleReactionUpdate = (payload: any) => {
      useChatStore.getState().updateMessage(sessionId, payload.messageId, { reactions: payload.reactions })
    }
    ...
    const handleMessageEdited = (payload: any) => {
      useChatStore.getState().updateMessage(sessionId, payload.messageId, { content: payload.content, edited: payload.edited })
