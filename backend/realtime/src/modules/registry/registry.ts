...
export class ConnectionRegistry {
  ...
  getConnectionByActorId(actorId: string, type?: "queue" | "chat"): ConnectionMetadata | undefined {
  ...
  updateMetadata(connectionId: string, updates: Partial<Omit<ConnectionMetadata, "connectionId" | "ws">>) {
    const conn = this.connections.get(connectionId);
    if (conn) {
      // If actorId changes, update the reverse index
  ...
  getAliveConnections(): ConnectionMetadata[] {
